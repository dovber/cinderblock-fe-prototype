...
export function EstimateEditor({ state, mode, initialSection, scenarioControl, onClose, onCreateInvoice, onCreateChangeOrder, onOpenChangeOrder, onOpenInvoice, onOpenStandardInvoice, onOpenReleaseInvoice, onRetainageDefaultChange, onCreateRetainageRelease, onAcceptEstimate, paymentScheduleContent }: Props) {
  ...
  return <EstimateSurface estimateId="1008" initialSection={initialSection} scenarioControl={scenarioControl} focused={mode === 'retainage'} showHeaderActions={false} onClose={onClose}
    sections={[
      ...
      { id: 'items', label: 'Items', content: <EstimateItemsSection lines={state.original} estimateLocked={estimateLocked} estimateLockReason={estimateLockReason} estimateDiscount={state.discount} contractDiscountPool={contractDiscountPool(state)} discountUsed={sum(state.invoices.filter(isPosted).map(invoice => invoice.discount))} discountReserved={sum(state.invoices.filter(invoice => invoice.status === 'Draft').map(invoice => invoice.discount))} taxRate={state.taxRate} costPlusPercent={state.costPlusPercent} costPlusTaxable={state.costPlusTaxable} draftByLine={state.original.map(line => sum(state.invoices.filter(invoice => invoice.status === 'Draft').map(invoice => invoiceAmounts(invoice, state.original)[line.id] ?? 0)))} lineContext={line => { const modified = lines.find(item => item.id === line.id)?.modifiedBy; const order = state.orders.find(item => `CO #${item.id}` === modified); return order ? <Button variant="text" onClick={() => onOpenChangeOrder(order)}>Modified by {modified}</Button> : null }}/>} ,
      ...(mode === 'regular' ? [
